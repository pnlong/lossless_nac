...
def predict_chunk(
    ...
    assert len(msb_tokens) == len(lsb_tokens), "MSB and LSB tokens must have the same number of samples"
    assert len(msb_tokens) == n_samples, "MSB and LSB tokens must have the same number of samples as the chunk"
    ...
    with torch.no_grad():
        for i in range(len(msb_tokens)):
            ...
            sequence[:, current_lsb_index] = predicted_lsb_token # set LSB token in input tokens
    ...
    # final assertions
    assert predicted_chunk.shape[0] == chunk.shape[0], f"Predicted chunk (n_channels={predicted_chunk.shape[0]}) must have the same number of channels as the original waveform (n_channels={chunk.shape[0]})"
