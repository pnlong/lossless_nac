...
for bit_depth in 8 16; do
    ...
    echo CUDA_VISIBLE_DEVICES="${gpu}" python "${software_path}" --gpu --audio_dir "${stereo_audio_dir}" --model_path "${llama_7b_path}" --bit_depth "${bit_depth}" --output_file "${outputs_dir}/stereo_llama2-7b_${bit_depth}bit_${max_length}.out" --max_length "${max_length}" --batch_size "${batch_size}" --stereo_blocking_n "${stereo_blocking_n}"
    echo CUDA_VISIBLE_DEVICES="${gpu}" python "${software_path}" --gpu --audio_dir "${stereo_audio_dir}" --model_path "${llama_13b_path}" --bit_depth "${bit_depth}" --output_file "${outputs_dir}/stereo_llama2-13b_${bit_depth}bit_${max_length}.out" --max_length "${max_length}" --batch_size "${batch_size}" --stereo_blocking_n "${stereo_blocking_n}"
    echo CUDA_VISIBLE_DEVICES="${gpu}" python "${software_path}" --gpu --audio_dir "${stereo_audio_dir}" --model_path "${llama_70b_path}" --bit_depth "${bit_depth}" --output_file "${outputs_dir}/stereo_llama2-70b_${bit_depth}bit_${max_length}.out" --max_length "${max_length}" --batch_size "${batch_size}" --stereo_blocking_n "${stereo_blocking_n}"
    echo
done