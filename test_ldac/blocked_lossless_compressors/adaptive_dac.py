...
def get_compressed_subframe_size(
    ...
    total_size = MAXIMUM_BLOCK_SIZE_ASSUMPTION_BITS

    # add size for DAC codes
    total_size += MAXIMUM_CODEBOOK_LEVEL_BITS # we can store the codebook level as one byte
    total_size += MAXIMUM_DAC_TIME_DIMENSION_ASSUMPTION_BITS # we can store the DAC time dimension as one byte, as a 1-byte unsigned integer
